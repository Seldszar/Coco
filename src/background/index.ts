...
import { getBountiesPage, getBountyBoardSettings } from "./twitch";
...
async function fetchBounties() {
  const responses = await Promise.all([
    getBountiesPage({ status: "AVAILABLE" }),
    getBountiesPage({ status: "LIVE" }),
  ]);

  return responses.flatMap<Bounty>(({ user: { bountiesPage } }) =>
    bountiesPage.edges.map(({ node }) => {
      const { campaign } = node;
